...
```text
CO(v_1, v_2) = sum_i min(v_1^i, v_2^i) / min(sum_i v_1^i, sum_i v_2^i)
```
...
```text
Dice(v_1, v_2) = 2 * sum_i min(v_1^i, v_2^i) / (sum_i v_1^i + sum_i v_2^i)
```
...
```text
Cosine(v_1, v_2) = sum_i v_1^i v_2^i / (sqrt(sum_i (v_1^i)^2) * sqrt(sum_i (v_2^i)^2))
```
...
```text
PCC(u_t, v_t) =
  sum_k (u_k,t - mean(u_t)) (v_k,t - mean(v_t)) /
  sqrt(sum_k (u_k,t - mean(u_t))^2 * sum_k (v_k,t - mean(v_t))^2)
```

**Euclidean Distance (`ed`)**

```text
ED(u_t, v_t) = ||u_t - v_t||_2
```
...
For a score `S_obs` and `R` surrogate scores `S_r`, the p-value is:

```text
p = (1 + count(S_r >= S_obs)) / (R + 1)
```
