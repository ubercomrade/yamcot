...
#### Tomtom-like Matrix/Tensor Comparison

The `tomtom-like` mode follows the matrix alignment idea of Tomtom [22].  
...
Compare motifs by direct N-dimensional matrix/tensor comparison (column-wise). Direct tensor comparison is intended for same-type models (e.g., BaMM vs BaMM, SiteGA vs SiteGA); heterogeneous pairs are compared via PFM reconstruction.
...
| `--pfm-mode` | Flag | Derive PFMs by scanning sequences, selecting best sites, and building site-based matrices before comparison (useful for heterogeneous model types). |
