...
To address these limitations, we introduce MIMOSA, a comprehensive framework designed to facilitate the comparison of diverse motif models beyond standard frequency matrices. MIMOSA exposes two comparison modes. The `profile` mode is the universal workflow: it compares TFBS recognition profiles, either from precomputed score tracks or from profiles generated by scanning sequences with motifs, conceptually similar to affinity-based approaches [25]. The `motif` mode performs direct matrix or tensor alignment for models with compatible representations and falls back to sequence-driven PFM reconstruction for heterogeneous model pairs (for example, BaMM vs PWM) [22], [26].
...
If you want to contribute to development or run the latest version from the repository, clone the project and install it in editable mode.
...
The `mimosa` tool provides two comparison modes and one cache-management command.
...
result = compare_motifs(
    model1=model1,
    model2=model2,
    strategy="profile",  # "profile" or "motif"
    sequences=sequences,
    metric="co",
    n_permutations=100,
    seed=42,
