...
from .comparison import DataComparator, MotaliComparator, TomtomComparator, UniversalMotifComparator
from .functions import scores_to_frequencies
...
class Pipeline:
    ...
    def execute_score_comparison(self, profile1_path: Union[str, Path],
                                profile2_path: Union[str, Path],
                                **kwargs) -> Any:
        """
        ...
        data_kwargs = {}
        for param in [
            'name', 'metric', 'n_permutations', 'distortion_level', 'n_jobs', 'seed',
            'filter_type', 'filter_threshold', 'search_range', 'min_kernel_size', 'max_kernel_size'
        ]:
            if param in kwargs:
    ...
    def load_sequences(self, seq_source: Union[str, Path, None],
                      num_sequences: int = 1000,
                      seq_length: int = 200) -> RaggedData:
        """
    ...
    def execute_tomtom_comparison(self, model1: MotifModel,
                                 model2: MotifModel,
    ...
    def execute_motif_comparison(self, model1: MotifModel,
                                model2: MotifModel,
    ...
    def run_pipeline(self,
        ...
        if comparison_type.lower() == 'profile':
            ...
            # Load models
            self.logger.info(
                f"Loading models from {model1_path} (type: {model1_type}) and "
                f"{model2_path} (type: {model2_type})"
            )
            ...
            result = self.execute_motif_comparison(
                model1, model2, sequences=sequences, promoters=promoters,
                comparison_type=comparison_type, **kwargs
            )
            ...
            raise ValueError(
                ...
                "Expected 'profile', 'motif', 'motali', or 'tomtom-like'."
            )


def run_pipeline(model1_path: Union[str, Path],
                 model2_path: Union[str, Path],
                 model1_type: str,
                 model2_type: str,
                 comparison_type: str = 'motif',
